...
static struct net_func_map {
  const char *name;
  void (*func)(Design *);
} func_table[] = {{"cprop", &cprop},         {"nodangle", &nodangle},
                  {"synth", &synth},         {"synth2", &synth2},
                  {"syn-rules", &syn_rules}, {0, 0}};
  ...
  while (fgets(buf, sizeof buf, ifile) != 0) {
    ...
    *cp++    = 0;
    ...
    if (strcmp(buf, "basedir") == 0) {
      ...
      } else if (strcmp(cp, "typecheck") == 0) {
        debug_typecheck = true;
        cerr << "debug: Enable type-checking debug" << endl;
      ...
      if (vpi_module_list == 0) {
        ...
        char *tmp = (char *)realloc(vpi_module_list,
                                    strlen(vpi_module_list) + strlen(cp) + 2);
        strcat(tmp, ",");
        strcat(tmp, cp);
        vpi_module_list = tmp;
      }
...
int main(int argc, char *argv[]) {
  ...
  if (version_flag) {
    cout << "\nIcarus Verilog Parser/Elaborator version " << VERSION << " ("
         << VERSION_TAG << ")" << endl
         << endl;
  ...
  if (help_flag) {
    cout << "Icarus Verilog Parser/Elaborator version " << VERSION << " ("
         << VERSION_TAG << ")" << endl
  ...
  if (flag_tmp)
    disable_virtual_pins = strcmp(flag_tmp, "true") == 0;

  flag_tmp = flags["ARRAY_SIZE_LIMIT"];
  if (flag_tmp)
    array_size_limit = strtoul(flag_tmp, NULL, 0);

  flag_tmp = flags["RECURSIVE_MOD_LIMIT"];
  if (flag_tmp)
    recursive_mod_limit = strtoul(flag_tmp, NULL, 0);
  ...
  if (typecheck_only) {
    ...
    if (des->errors) {
      cerr << des->errors << " error(s) in post-elaboration processing."
           << endl;
      return des->errors;
    }
    ...
    if (int emit_rc = des->emit(&dll_target_obj)) {
      if (emit_rc > 0) {
        cerr << "error: Code generation had " << emit_rc << " errors." << endl;
        return 1;
      }
      if (emit_rc < 0) {
